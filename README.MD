...
### Quick links:
https://trofi.one is the production server. This serves the frontend. There are no backend operations as these are all handled using IPFS, web3 integration, and thegraph.com for query caching and indexing. The production server pulls and rebuilds the frontend client whenever a pull request is merged.
